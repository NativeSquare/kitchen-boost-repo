...
> **Slices** : #458 (S7 Stripe payment + latching surge + saved-card branch — PR #478) · **Statut** : 🟡 EN COURS — blocker config Stripe **RÉSOLU 2026-06-12**, Payment Element rendu ✅ ; PAY.1/2/3 en cours d'exécution.
>
> **✅ Blocker config Stripe RÉSOLU (2026-06-12)** : le Payment Element rendait une **div vide** (console : `GET https://api.stripe.com/v1/elements/sessions?...&type=deferred_intent → 403 (Forbidden)` + `Unhandled payment Element loaderror`). **Cause racine (confirmée via API Stripe)** : **mismatch de comptes** entre la **publishable key** Vercel (`pk_test_51Sffl**K**…`, plateforme K) et la **secret key** Convex (`sk_test_51Sffl**V**…`, plateforme V). Le compte connecté `acct_1TgBUq4DuUOJbMhp` appartient à V (`charges_enabled:true`, `card_payments:active`, 0 requirement). Client init Stripe.js avec pk de K + `Stripe-Account: acct_1TgBUq` (non possédé par K) → 403. **Le code S7 était correct** — pur problème d'env. **Fix appliqué** : `NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY` corrigée dans Vercel vers la publishable de la plateforme V (`pk_test_51SfflV34XeEJ…`, même segment compte que la secret) + redeploy `main`. Après hard-refresh : formulaire carte Stripe rendu ✅. **Leçon** : la pk avait été posée dans Vercel après le 10/06 avec la clé d'un autre compte ; toujours vérifier que pk et sk partagent le même segment compte (`pk_test_51XXXX` == `sk_test_51XXXX`).
>
