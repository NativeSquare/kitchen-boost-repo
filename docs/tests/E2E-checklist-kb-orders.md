...
> **Statut** : ✅ **4/4 PASS le 2026-06-07** — KBO-DIS1 + KBO-DIS2 + KBO-DIS3 + KBO-DIS4 FULL. KBO-DIS4 (modif horaires #409) post-`9042f80` : pivot wheel picker custom maison (zéro dep, 120 LOC ScrollView + snapToInterval, vert KB band) — résout le bug `timeStringToMinutes` regex strict qui rejetait les inputs partiels + commenté widget Notifications dans Paramètres V1 (préservé pour V2). Suit `599c2b8` (retrait tab + crash navigation). Dette UX restante : retrait du bouton "Enregistrer" global + persistance unitaire au "Valider" du wheel sheet (en cours). Fixs livrés pendant KBO-DIS3 : `f016467` (badge état + wording + `withOptimisticUpdate` + per-item Set + PRD §7c update asymétrie OFF only) + `04c596b` (touch area row Pressable + retrait ActivityIndicator anti-optimistic — cause racine identifiée : Switch remplacé conditionnellement, masque l'illusion d'instantanéité) + `9471a74` (Switch `@rn-primitives/switch` wrap dans View `pointerEvents="none"` → thumb dans la touch area + 2 nouvelles toast entries `menu.itemSetAvailable` / `menu.itemSetUnavailable` wirées post-mutation success avec itemName en sous-ligne). Fixs livrés pendant le run : `599c2b8` (pause custom + retrait tab horaires + fix crash accessibilityRole=tab) + `6e6181e` (bannière availability-banner) + `0b1e3df` (helper closureScheduled kind quand `from > now` — bug bannière hidden sur custom future, +picker DateTimePicker natif initial) + `70252ba` (pivot calendar JS-only `react-native-calendars` Wix car `@react-native-community/datetimepicker` crash module natif non-bundlé dans dev build EAS — +titres header drawer dédupliqués "Commandes en cours" / "Stats rapides" + fix padding bannière `useSafeAreaInsets` consommé une fois sur wrapper bannières + `headerStatusBarHeight: 0` drawer pour éviter double inset). Vérif `acceptsOrderNow` : 36 tests pinned (status.test.ts) couvrent pause + closure + hours + cross-tenant + auto-reprise — RAS côté backend. Issues couvertes #406, #407, #408, #409.
...
> **Statut** : 🟢 **4/4 PASS le 2026-06-07** — KBO-OPS1 PASS (stats rapides #410, grille 2×2 + comparator emerald/rose/gris/Nouveau + Convex sub temps réel). KBO-OPS2 PASS (impression Star WebPRNT #412 — validation URL stricte client+serveur exigeant `/StarWebPRNT/SendMessage` (fix `http://invalid`), save+test+auto-impression au accept+réimpression manuelle+retirer avec Alert natif, toasts `printer.save / testOk / remove / reprintOk / reprintError` ; mock Node `scripts/star-webprnt-stub.mjs` reçoit les 3 tickets — ESC/POS `Test Restaurant 1 / Cmd #6EVF / LIVRAISON / items + modifiers / note client / Total 27,50 EUR` byte-identique sur la réimpression). KBO-OPS3 PASS (historique #417 — 4 onglets `Toutes / Livrées-Collectées / Refusées / Manquées` × 4 périodes `Aujourd'hui / 7j / 30j / Tout` × recherche ID-only MOAT ADR 0010 ; badges sémantiques vert positif / rouge négatif ; tap row → détail readonly ; seed `seedE2EKBOrdersHistoryMix` panache 9 terminaux sur today/-3j/-10j/-40j ; anti-MOAT confirmé). KBO-OPS4 PASS (settings #418 — 9 sections Header / Ouverture & horaires → `/settings/availability` / Profil (« Modifier » seul, **MDP retiré** car géré depuis KB Admin web décision terrain 2026-06-07) / Compte rattaché (tenant + badges Stripe/Uber + bouton « Ouvrir KB Admin web » → `https://admin.kitchen-boost.com` (fix domaine `.com` + sous-domaine `admin`)) / Imprimante → `/printer` / Mode app (toggle kiosque ↔ téléphone via ConfirmationSheet) / À propos (version + build + Play Store) / Support (mailto:support@kitchen-boost.com) / Se déconnecter (ConfirmationSheet rouge destructive) ; **Notifs V1 absent** confirmé). Issues couvertes #410, #412, #417, #418.
...
> **Statut** : 🟢 **PASS le 2026-06-07** — toutes les surfaces admin propagent live au natif via Convex sub. **A #397 dispo commerciale** : pause/closure/items/horaires depuis `/t/<id>/disponibilite` reflètent en ≤3s sur l'emulator (mirror app native bidirectionnel). **B #415 historique + onglet Manquées** : 4 onglets mirror du natif sur `/t/<id>/commandes`, badges sémantiques, alerte ops conditionnelle au seuil `auto_expired > N` (par défaut pas affichée avec 1 cmd seed OPS3). **C #416 config imprimante par tenant** : URL admin sync bidirectionnelle avec l'écran natif `/printer`, même `tenants.printerConfig.starWebPrntUrl` côté backend. **D #396 sessions + révocation** : RBAC durci kb_admin only (décision terrain — pré-fix un kb_manager pouvait booter latéralement un autre manager du même tenant). Défense en profondeur 3 layers : backend `tenantQuery/Mutation({allow: []})` + sidebar entry hidden pour kb_manager + page `<UnauthorizedCard/>` si URL forgée. Watcher live `isMySessionAlive` (`getAuthSessionId(ctx)` → check existence row) résout le bug E2E sur la propagation (Convex Auth garde le JWT côté client jusqu'à expiration TTL ~1h → la sub bypasse cette latence) — overlay rouge `<SessionRevokedGate/>` flip en ≤5s. Issues couvertes #396, #397, #415, #416 (+ collat #400). Commits clés : `5fd9b94` (RBAC sessions + dedup react 19.2.3), `d8cfe2b` (watcher live isMySessionAlive).
