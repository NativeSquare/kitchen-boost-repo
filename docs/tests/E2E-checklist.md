...
> **Statut partiel** : ✅ **MC1-MC16 validés le 2026-06-01 → 2026-06-02** (vagues MC-A KPIs + MC-B Campagnes liste + picker + MC-C Template form + violation + MC-D Send + anti-anomaly + cross-tenant). Seeds : `seedE2ECustomerKPIs` + `seedE2ECampagnesTemplates` + `seedE2ECampagnesCorruptedTemplate` + `seedE2EMCSendCustomers` + `seedE2EMCAnomalyLaunch`. 2 fixes critiques pendant la run : `fd084ef` (barrel notifications leakait `whoAmI = query()` côté browser → fix import direct des sous-modules purs) + `2cece86` (seeds customer ne stampaient pas `cgvAcceptedAt` → tous les sends MC14 tombaient dans `skippedIneligible`). 2 gaps produit logués : **#387** (nom_resto + prenom_client doivent être auto-inject, pas saisis par le gérant — bloquant V1) + **#388** (spot-check real delivery Wallet APNS + Resend prod requis avant déploiement V1 chez vrai resto — wiring testé mais pas la livraison physique). Reste à valider : MC17-MC19 (vague MC-E, historique — seed `seedE2EMCT2Launch` à ajouter pour MC18), MC20-MC27 (vague MC-F, dashboard + stats + LineChart — seed orders à ajouter).
