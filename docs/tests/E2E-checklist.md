...
> **Statut partiel** : ✅ **MC1-MC19 validés le 2026-06-01 → 2026-06-02** (vagues MC-A KPIs + MC-B Campagnes liste + picker + MC-C Template form + violation + MC-D Send + anti-anomaly + cross-tenant + MC-E Historique). Seeds : `seedE2ECustomerKPIs` + `seedE2ECampagnesTemplates` + `seedE2ECampagnesCorruptedTemplate` + `seedE2EMCSendCustomers` + `seedE2EMCAnomalyLaunch` + `seedE2EMCT2Launch`. 3 fixes critiques pendant la run : `fd084ef` (barrel notifications leakait `whoAmI = query()` browser) + `2cece86` (seeds customer cgvAcceptedAt manquant → tous sends MC14 skippedIneligible) + `69b0a97` (historique crash sur launchId mal-formé → guard `isLikelyConvexId` + skip query). 2 gaps produit logués : **#387** (nom_resto + prenom_client auto-inject — bloquant V1) + **#388** (spot-check real delivery Wallet APNS + Resend prod requis avant V1 réel). Reste à valider : MC20-MC27 (vague MC-F, dashboard + stats + LineChart — seeds orders T1/T2 à ajouter ; MC24 staff = spot-check code-review, même page que MC22 avec allow-list différente déjà testée en unit).
