...
> **Statut** : ✅ **26/27 validés le 2026-06-01 → 2026-06-02** (MC24 staff Stats marqué spot-check code-review — même page que MC22, allow-list `["kb_manager", "staff"]` testée en unit ; provisionner un user staff réel demanderait extension de `bootstrapE2EInvites`, out of scope). 6 vagues : MC-A KPIs + MC-B Campagnes picker + MC-C Template form + MC-D Send + anti-anomaly + MC-E Historique + MC-F Dashboard/Stats/LineChart. Seeds : `seedE2ECustomerKPIs` + `seedE2ECampagnesTemplates` + `seedE2ECampagnesCorruptedTemplate` + `seedE2EMCSendCustomers` + `seedE2EMCAnomalyLaunch` + `seedE2EMCT2Launch` + `seedE2EOrdersT1` + `seedE2EOrdersT2`. 4 fixes critiques : `fd084ef` (barrel notifications leakait `whoAmI = query()` browser) + `2cece86` (seeds customer cgvAcceptedAt manquant → tous sends MC14 skippedIneligible) + `69b0a97` (historique crash sur launchId mal-formé → guard `isLikelyConvexId`) + `162bb5f` (sidebar manquait « Tableau de bord » + « Statistiques », pages inaccessibles sans forger URL + 4 placeholders « (story 5-8) » commentés). 4 gaps produit logués (tous bloquants V1) : **#387** (nom_resto + prenom_client auto-inject) + **#388** (real delivery spot-check Wallet APNS + Resend prod) + **#389** (4 cartes placeholders avec nomenclature dev — fix appliqué via commentaire JSX, dé-commenter quand slices F-STATS 5-8 atterrissent) + **#390** (sidebar entries Tableau de bord + Statistiques — fix appliqué).
...
> **Statut** : ✅ **25/25 validés manuellement le 2026-06-01** (vagues T-A switcher + T-B supervision + T-C contrats + T-D Kanban DnD + T-E fiche détaillée). T5 + T8 + T10 variante (a) marqués spot-check code-review (branches non-déterministes en e2e propre). 4 commits pendant les runs : `730c5fc` (render Markdown → HTML via `marked` + style inline) + `b3ad04d` (5 champs juridiques éditables + validation SIRET/email + persist via `editProspect` + descriptions FR Prestation A/B/A&B) + `a715b66` (modal responsiveness : `max-w-2xl` + sticky header/footer + body scroll interne) + `3418154` (DragOverlay portal + clone — cause racine T17 : pas de `<DragOverlay>` du tout, `CSS.Translate` posé sur le noeud in-flow → carte clippée par `overflow-y-auto`). Seeds T-B/C/D/E ajoutés à `convex/e2e.ts` : `seedE2ESupervisionProspects` + `seedE2EContractProspects` + `seedE2EKanbanDnDProspects` + `seedE2EFicheProspectDetails`.
...
> **Statut M** : ✅ **15/15 validés le 2026-06-03** (M-A 4/4 mode blank : M1/M6/M7/M9bis + M-B 11/11 mode full : M2/M2bis/M3/M3bis/M4/M5/M8/M8bis/M8ter/M9/M9ter). Seeds : `seedE2EMenuT1 {mode:"blank"|"full"}`. **7 fixes critiques UX shippés pendant cette vague** (Alex testait, signalait, sub-agent fixait, push) : **1ed33a4** (M1 catégorie : `<Input>` n'avait pas de `onKeyDown` Enter handler + la row fraîchement créée n'autofocusait pas → Enter flush + autofocus + select-all on new row) + **07af356** (modale item Dialog centré → `Sheet` right-drawer `sm:max-w-xl`) + **c5dd031** (padding interne `Sheet`) + **4170f13** (`ModifiersSection` gated sur `mode === "edit"` → invisible en create ; en plus le picker était texte + boutons séparés ; refonte : chips inline + popover dropdown filtrable + DnD réorder via dnd-kit + visible en mode create avec `pendingAttachedGroupIds` chained-attach après création + backend nouvelle mutation `modifiers.reorderItemGroups`) + **9dfc03b** (prix options personnalisation centimes brut → format `0,00 €` + validation inline négative/format/2 décimales + mode create + edit) + **db8e295** (page menu single-screen scrollable → split en 3 tabs **Catégories / Plats / Personnalisations**, default = Plats, URL state `?tab=...` bookmark-safe, header [badge unpublished + Aperçu + Publier] hors-tabs, modale item partagée à travers les tabs) + **ea397fb** (M-B révèle 2 bugs : (1) tab Plats avait les headers cat éditables car réutilisait `CategoryListEditor` mode `with-items` — nouveau `displayMode="items-only"` rend les headers en `<h3>` uppercase non-éditable, supprime Input/poubelle/drag-handle catégorie + footer « + Catégorie », DnD intra-cat items + « + Item » conservés ; (2) bouton Aperçu fonctionnel mais cache Next.js corrompu côté Alex provoquait l'illusion de bug — test contractuel anti-régression ajouté pour lock le câblage `previewHref` sur les 3 tabs).
...
> **Statut** : ✅ **10/10 validés le 2026-06-02 → 2026-06-03** (W-A spot-check W1/W2/W3 + W-B parcours complet W4/W6/W6bis/W7/W8/W10 + W-C standalone W5 Stripe Connect KYC ; W9 Invitation gérant reporté à AC2/AC2bis — couverture identique). Seeds : `seedE2EWizardProspect` (slug prefix `e2e-w-`). 6 fixes critiques pendant cette vague : **fe52361** (steps 3/6 marqués ✅ par défaut → flags `step{3,6}Visited` mirror `step2Skipped`) + **1d2f376** (manager qui hit URL wizard recevait `ConvexError` raw → skip-sentinel sur 4 queries `useWizardState` + `UnauthorizedCard` early return) + **c16fd6f** (W8 step 6 utilisait pipeline PDF obsolète → migration vers shared `QrDownloadCard` SVG, suppression jspdf) + **9ef58a1**/**4daf357** (W7 input catégorie controlled value+defaultValue + useEffect resync wipait la frappe pendant le debounce 600ms → seed once via useState, no resync) + **204b692**/**bd5ad92** (W7 step 5 publish auto-advançait vers step 6 — cause : `useWizardState` calculait `currentStep = override ?? liveState.currentStep` et `liveState.currentStep` sautait à 7/8 quand `publishMenu` flippait `step5Complete=true` → remplacé `override` par cursor explicite seedé une fois, ensuite mutable uniquement via `goToStep`) + **d601a2d** (W5 Stripe `createStripeAccountLink` envoyait `business_type:"company"` + params `individual[first_name/last_name/email]` simultanément → Stripe rejetait `« You can only provide individual parameters for accounts with business_type of 'individual' »` ; fix = retirer les 3 params `individual[*]`, le KYC Express UI collecte le représentant légal lui-même). 1 gap produit logué (P1, non bloquant V1) : **#391** (pas de vraie vérification DNS du customDomain avant activation — caveat dans W4). **Scope W5 réduit** : test = mutation + URL + reuse `acct_xxx` ; le KYC sandbox + webhook `account.updated` ne sont PAS testés en E2E manuel (services tiers — cf. mémoire `e2e-scope-what-we-control`).
