...
> **Statut partiel** : ✅ **T1 + T1bis + T2 + T3 + T4 + T6 + T7 + T9 + T10 + T11 + T12 + T13 + T17 + T18 + T19 validés le 2026-06-01** (vagues T-A switcher + T-B supervision + T-C contrats + T-D Kanban DnD). T5 + T8 + T10 variante (a) marqués spot-check code-review (branches non-déterministes en e2e). 4 commits pendant les runs T-C/T-D : `730c5fc` (render Markdown → HTML via `marked` + style inline) + `b3ad04d` (5 champs juridiques éditables + validation SIRET/email + persist via `editProspect` + descriptions FR Prestation A/B/A&B) + `a715b66` (modal responsiveness : `max-w-2xl` + sticky header/footer + body scroll interne) + `3418154` (DragOverlay portal + clone — cause racine T17 : pas de `<DragOverlay>` du tout, `CSS.Translate` posé sur le noeud in-flow → carte clippée par `overflow-y-auto`). Reste à valider : T14-T16 + T20-T25 (vague T-E, fiche prospect détaillée — seed à ajouter).
