...
**Allergènes** :
Déclaration des allergènes potentiels par item. **V1 = compliance stricte Règlement UE 1169/2011** (Q10-Q6 acté 2026-05-24) : 14 allergènes obligatoires à déclarer pour la vente à distance (livraison) — `gluten, crustacés, œufs, poissons, arachides, soja, lait, fruits à coque, céleri, moutarde, graines de sésame, sulfites, lupin, mollusques`. **Littéral canonique « graines de sésame »** (jamais « sésame » seul — acté 2026-05-26, recon 2.2 : schéma `menuItems.allergens` et CONTEXT convergent sur cette chaîne exacte dans l'union figée). **Paramétrable item par item depuis [[KB Admin]]** par le resto (matrice items × 14 cases à cocher, saisie en Phase C onboarding). Affichage côté PWA : badges allergènes sur fiche item détail + filtre simple "Sans gluten / Vegan / Végé" sur la home menu. **V1 = filtre FRONT-SIDE uniquement** (acté 2026-05-26, recon 2.2, PRD 10 §5) : `getPublicMenu` renvoie le set des 14 allergènes **par item**, et le filtre Sans-gluten/Vegan/Végé est appliqué côté PWA par-dessus ce set — **pas de query de filtrage backend, pas d'argument de filtre** sur l'endpoint en V1. Couvre la responsabilité légale du resto (qui reste responsable de la véracité des déclarations). V2 envisagera import auto via fiches recettes Gimmy/Taster si format standardisé.
...
Texte d'accroche + code promo associé à l'install du [[Wallet pass]], configurés en Phase C onboarding depuis [[KB Admin]] et modifiables à tout moment par le resto. Exemples : "🎁 Reçois -10% sur ta prochaine cmd" / "🥤 1 Cristaline offerte" / "🍰 1 dessert offert". **Code promo généré et appliqué uniquement si pass effectivement installé** (event `pass_installed` via webhook Apple/Google côté backend) — pas de fake reward. **V1 = [[Push enrollment]] BLOQUANT à la validation paiement** (révision Q10-Q acté 2026-05-24) : bouton "Payer" disabled tant que le client n'a pas activé au moins un canal push (Wallet pass OU Web Push OU A2HS). **3 paliers de prompt Wallet avant le bloquant** (acté grilling PWA Q5 2026-06-04, cf. [docs/prd/sub/10-pwa-client/00-master.md](../../prd/sub/10-pwa-client/00-master.md)) : (1) **soft prompt** card pleine page après submit address-first ("🎁 -10% sur ta prochaine cmd → Ajoute la carte" + bouton primary + "Plus tard" skippable) ; (2) **bandeau permanent** menu/panier ("🎁 -10% offerts → ajoute la carte", dismissable session-scoped) ; (3) **modal bloquant** au clic Payer (single-screen Wallet primary + Web Push secondary, non-skippable Esc/click outside). Modal incontournable au paiement : "Pour finaliser ta cmd, choisis comment recevoir ta confirmation + offres : 🥇 Carte fidélité Wallet (2 taps) / 🥈 Notifs navigateur (1 tap)". Trade-off conversion iOS assumé (~10-15% drop possible) pour préserver le moat 100%. **Fallback "Continuer sans notifs"** = 3 niveaux frictionnels après 2 échecs documentés (lien microscopique 12px → modal confirm "AUCUNE confirmation, AUCUN suivi" → re-display modal initial) ; re-refus final = flag `noChannelPossible` + SMS fallback transactionnel via Cascade Notifications.
_Avoid_: Promo install, Reward A2HS, Surprise

**Bandeau de retour** (Recognition retour 2ᵉ visite) :
Bandeau accueillant `"Bonjour {firstName} 👋"` affiché au-dessus du form Google Places sur `/` à la 2ᵉ visite si `firstName` est connu (capté à un checkout précédent terminé). Règle UX **non-négociable** (acté grilling PWA Q3 2026-06-04) : **JAMAIS mention adresse/historique explicite** dans l'UI ("On a ton adresse" / "Voici tes 4 dernières cmds" interdits — perception surveillance vs hospitalité). Adresse pré-remplie silencieusement dans le form, bouton form passe de "Valider" à "Confirmer" si pré-rempli. Lien discret `"Ce n'est pas moi →"` (font-size 12px, color muted) en-dessous du form → clear cookie Convex Auth uniquement + refresh page (cookie tenant inchangé). Si cookie présent mais `firstName === null` → AUCUN bandeau, juste pré-rempli silencieux. Reconnaissance retour cross-resto / cross-device uniquement via [[Wallet pass]] bridge (cf. [[Identité Customer V1]] dans [[Customer Data]]).
