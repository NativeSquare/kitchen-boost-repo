...
**Push enrollment** :
Action explicite du client pour autoriser KB à lui pousser des notifs. **3 mécanismes V1** : Add to Wallet (2 taps), Allow notifications browser (1 tap, Android sans A2HS), A2HS install (1-4 taps selon plateforme). Maximiser le push enrollment = objectif V1 prioritaire pour construire le [[Lock-screen reach]]. **V1 = BLOQUANT à la validation paiement** (PRD 10 §9) précédé de **3 paliers de prompt Wallet progressifs** (acté grilling PWA Q5/Q8 2026-06-04, cf. [docs/prd/sub/10-pwa-client/00-master.md](../../prd/sub/10-pwa-client/00-master.md)) : (1) **soft prompt** card pleine page juste après submit address-first (skippable "Plus tard") ; (2) **bandeau permanent** menu/panier (dismissable session-scoped) ; (3) **modal bloquant** au clic Payer single-screen Wallet primary + Web Push secondary (non-skippable Esc/click outside, option Web Push masquée si iOS <16.4 via `'PushManager' in window === false`). Fallback 3 niveaux frictionnels après 2 échecs documentés → flag `customer.pushEnrollment.noChannelPossible = true` + SMS fallback transactionnel via Cascade Notifications. Détection canal actif côté front : Convex subscription sur `customers.pushEnrollment.{walletStatus, webPushStatus, a2hsStatus}` — bouton Payer débloque dès qu'au moins UN est `"enrolled"` (realtime, sans reload).
...
Pattern type Firebase Anonymous Auth / Spotify : à la première arrivée sur la PWA, on crée silencieusement un `customer_id` et un **token persistant côté device** (cookie HttpOnly long-lived) — pas de password, pas d'écran "créer un compte", pas de friction. **Timing signIn = au submit du form address-first** (acté grilling PWA Q3 2026-06-04, option C) — pas au mount client, pas dans le middleware Edge. Coïncide avec la 1ère écriture utile (address + lat/lng + déclenche [[Quote]] Uber). Évite la pollution de la table `users` par bots et visiteurs qui rebondissent (~90% du noise éliminé). **Session lifetime = 365j total + 365j inactive** (sliding window, override Convex Auth default 30j via `convexAuth({ session: { totalDurationMs: 365*24*3600*1000, inactiveDurationMs: 365*24*3600*1000 } })` dans `auth.ts`). Trade-off assumé : 2ᵉ visite > 1 an cookie expiré → re-submit adresse (Google Places ~5s). Reconnaissance silencieuse à la session suivante **sur le même device** dans la fenêtre 365j. **Cross-device V1 = uniquement via [[Wallet pass]] installé** (cf. [[Identité Customer V1]] + [ADR 0008](../../adr/0008-identite-customer-cookie-device-only-v1.md)) ; sans pass installé, un nouveau device = un nouveau `customer_id`. **Canaux push V1 dual** (cf. [[Wallet pass]] dans [[Client Ordering]]) : (1) Apple/Google Wallet pass = primary moat, 2 taps install, push lock-screen 85-99% open rate, sert AUSSI de bridge cross-device pour l'identité ; (2) A2HS PWA = bonus, requis pour push web iOS 16.4+, scopé device. Le client n'a pas conscience d'avoir un compte — il est techniquement identifié sur son device (et cross-device s'il a le pass). UX retour : pré-remplissage via RSC `convexAuthNextjsToken()` + `preloadQuery(getCurrentCustomer)`, bandeau "Bonjour {firstName} 👋" si firstName connu (cf. [[Bandeau de retour]] dans [[Client Ordering]]).
