...
>
> **v0.3.1 (2026-05-25)** : chaque resto est sur son **propre domaine de marque** (jamais un sous-domaine KB côté public). Donc le **cookie device est intra-resto**, et la **reconnaissance cross-resto repose EXCLUSIVEMENT sur la carte Wallet** ([ADR 0008](../adr/0008-identite-customer-cookie-device-only-v1.md) Amendement 2026-05-25). Table `customerSessions` / POC #4 « cookie domaine parent » = **abandonnés** (prémisse sous-domaine caduque).
>
> **v0.2** : Scope V1 étendu suite révision master v2.0. **Position géo dès V1**. **Partage manuel KB → resto** repoussé V2. **Multi-resto effet réseau** = carte Wallet (cf. v0.3.1).
...
- **Session = cookie Convex Auth natif, intra-resto** (un domaine de marque par resto). Pas de table `customerSessions` ni de cookie « domaine parent » : la prémisse sous-domaine KB est caduque et la reconnaissance cross-resto passe par la carte Wallet, pas par la session ([ADR 0008](../adr/0008-identite-customer-cookie-device-only-v1.md) Amendement 2026-05-25).
...
**Révisé 2026-05-25 ([ADR 0008](../adr/0008-identite-customer-cookie-device-only-v1.md) Amendement)** : reconnaissance par **cookie device intra-resto** + **carte Wallet pour le cross-resto** (PAS de match email/tel). Pas de message brandé « compte KB » côté client.

- Client commande chez 1er resto → création `customer` (user anonyme) sur le domaine de ce resto.
- Client revient **sur le même resto, même device** → le cookie reconnaît le même `customer` → auto-fill prénom + adresse. Pas de prompt « tu es déjà client ? ».
- Client va chez un **2e resto** (autre domaine de marque) → par défaut **nouveau `customer`** (cookies cloisonnés entre domaines). Reconnu comme le même `customer` cross-resto **uniquement** s'il a installé la carte Wallet (pont `serial → customer`).
- Client sur un **autre device** → même règle : reconnu uniquement via la carte Wallet ; sinon nouvelle fiche (doublon assumé ~10 %).
