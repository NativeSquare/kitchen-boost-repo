...
- **Banner cookie V1** : first-party cookies essentiels uniquement (customer_id token), CNIL exempte. Checkbox RGPD consent_marketing au checkout = bloquant (ADR 0001). V2 si analytics tiers branchés.
...
- Si client déjà connu (Anonymous account cookie OU match silent email/tel cross-device) → pré-remplissage complet
- RGPD : checkbox explicite consent_marketing **bloquante** (cf. [ADR 0001](../adr/0001-consent-marketing-bloquant-checkout-v1.md) + [90_donnees_clients_crm.md](90_donnees_clients_crm.md))
...
|---|----------|----------|-------|
| 10-Q1 | ~~Cookie consent banner obligatoire ?~~ **ACTÉ 2026-05-24** : V1 = first-party cookies only (customer_id token + session), pas de banner cookie (CNIL exempte). Checkbox RGPD consent_marketing au checkout = bloquant (ADR 0001). V2 si analytics tiers branchés. | — | — |
