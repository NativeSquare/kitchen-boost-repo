...
const ALL = [KYC, WEBHOOK, PAID_KHAN, PAID_KIM, PAID_ORPHAN];
...
describe("filterIncidents — F-MONITORING (#197)", () => {
  ...
  it("filters by kind exactly — `paid_no_course` keeps every paid-no-course row including the orphan", () => {
    const out = filterIncidents(ALL, {
      kind: "paid_no_course",
      tenantId: "all",
      severity: "all",
  ...
  it("filters by severity « critical » → keeps `webhook_latency` + `paid_no_course`, drops `kyc_pending` (warning)", () => {
    const out = filterIncidents(ALL, {
      kind: "all",
      tenantId: "all",
      severity: "critical",
  ...
  it("filters by severity « warning » → keeps `kyc_pending` only", () => {
    const out = filterIncidents(ALL, {
      kind: "all",
      tenantId: "all",
      severity: "warning",
  ...
  it("filters by tenant id — keeps only the tenant-scoped incidents matching that id", () => {
    const out = filterIncidents(ALL, {
      kind: "all",
      tenantId: "tenant_khan",
      severity: "all",
    });
    expect(out).toEqual([PAID_KHAN]);
  });
...
describe("collectTenantOptions — F-MONITORING (#197)", () => {
  ...
  it("collects distinct tenant ids from `paid_no_course` incidents (the only tenant-scoped kind in the union)", () => {
