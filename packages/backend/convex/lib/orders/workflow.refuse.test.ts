...
describe("2.3-E refuse — nouvelle → refusée + refund order emitted (atomic)", () => {
  ...
  it("emits the client refund_issued notification event (queued — sending is 2.7)", async () => {
    ...
    expect(notifs.every((n) => n.transactionalTrigger === "refund_issued")).toBe(
      true,
    );
    ...
    expect(notifs.every((n) => n.tenantId === seed.tenantA.tenantId)).toBe(true);
  });
  ...
  it("refusing from a non-nouvelle state is rejected by the state machine (only nouvelle → refusée)", async () => {
    const asManager = t.withIdentity({ subject: seed.tenantA.managerId });
    for (const status of [
      "en préparation",
      "prête",
