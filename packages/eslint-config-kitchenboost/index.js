 ...
 *
 * `no-untenanted-query` fires on raw `ctx.db.{query,get,insert,patch,replace,
 * delete}` in business code. The DOCUMENTED EXEMPTIONS below (ADR 0010, grill
 * 1.x decisions C/D) are the only sanctioned places raw `ctx.db` is allowed —
 * they are transverse plumbing / GLOBAL (non-tenant-scoped) tables / generated
 * code / tests, NOT tenant-scoped business data. Every exemption is path-based
 * so new business modules (the 2.x code this gates) get NO escape hatch and must
 * go through `tenantQuery` / `tenantMutation` / `kbAdminQuery`.
 */
plugin.configs.backendRecommended = [
  {
    ...
    ignores: [
      ...
      "convex/lib/tenancy/**",
      // The single sanctioned identity point (getCurrentActor, ADR 0011): reads
      // the GLOBAL users / userTenants tables to resolve the actor.
      "convex/lib/auth/**",
      ...
      // (owned by Convex Auth, not tenant-scoped). Conformed where it mattered
      // (the unguarded role-escalation footgun was removed in 1.x-A); the
      // remaining raw access is the sanctioned global-table foundation.
      "convex/table/**",
